...
from exchange_lib.util.api_lib import generate_nonce, generate_signature
...
class WebSocketLagError(Exception):
    """
    Error upon ws lag
    """
    pass
...
class BitMEXWebsocket(Event):
    ...
    def __on_message(self, message, k=None):
        ...
        try:
            if 'subscribe' in message:
                ...
                if table == 'quote' and len(self.data[table]) and self.started:
                    self.last_timestamp = datetime.utcnow()
                    ticker = self.get_ticker()
                    self.lag = (datetime.utcnow() - datetime.strptime(ticker['timestamp'],
                                                                      '%Y-%m-%dT%H:%M:%S.%fZ')).seconds

                    if self.lag >= 30:
                        self.logger.critical('bitmex is lagging {} second(s)'.format(self.lag))
                        self.lagging = True
                        #raise WebSocketLagError('Websocket Closed due to lag!')
                        self.logger.error(f'Websocket is lagging {self.lag} seconds.')
                    else:
                        self.emit('quote_update', self.get_ticker())
        except:
